...
def create_index(service_endpoint, admin_key, index_name):
    ...
    index_fields = [
        ...
        SearchableField(name="content", type="Edm.String",searchable=True),# this field for remove html
        SimpleField(name="sys_id",type="Edm.String",searchable=True),
...
def upload_documents(documents, service_endpoint, admin_key, index_name,limit=None):
    try:
...
if __name__ == "__main__":
    try:
        ...
        all_records = get_all_records(limit=30)
        ...
        documents = []
        for record in all_records.get("result", []):
            if isinstance(record, dict):
                descriptions = record.get("text", "")
                content_without_html = remove_html_tags(descriptions)
                ...
                # Inside the loop where you generate embeddings for content
                content_data = content_without_html  # Assuming content_without_html contains your content data
                ...
                document = {
                            ...
                            "content": str(content_without_html), # Wrap content in a list
        ...
        upload_documents(documents, service_endpoint, admin_key, index_name,limit=100)
