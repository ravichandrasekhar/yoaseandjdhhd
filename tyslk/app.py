...
async def init_openai_client():
    ...
    try:
        ...
        print("Azure_openaiendpoint",endpoint)
        # Authentication
        ...
        print("azure openai endpoint details",azure_openai_client)
        print("azure openai key",aoai_api_key)
  ...
  **Subject:** Regarding your inquiry (summarize this {query} here)
  ...
  """
    ...
    def get_highlight_info_prompt(question,generated_answer,documents_used,doc_references): 
        ...
        STRICTLY extract and provide exact unmodified relevant line from each of the documents provided above while returning the lines ensure to keep intact in lines any newline charachet by \\n if present in given document,  If you are not able to find specific relevant lines from a specific document to generate the answer, simply provide an empty JSON object for that document, do not generate a line from your own for response.
    ...
    #         try:
    #             response = get_openai_response(messages,app_settings.azure_openai.model,json_mode=False)
    ...
    #                         documents = documents[:-1]  # Reduce the number of documents by 1
    #                     else:
    #                         raise e
    ...
    return response
def highlight_citations_in_response(response, citations):
    """
    Highlights citations in the given response by appending metadata.

    Args:
        response (dict): The formatted response.
        citations (list): List of citation metadata.

    Returns:
        dict: Updated response with highlighted citations.
    """
    if not citations:
        return response
    ...
    for citation in citations:
        i = i + 1
        references = references + ' [doc' + str(i) + ']'
        if (str(response.choices[0].message.content).find('[doc' + str(i) + ']') != -1):
            citation['background'] = 'lightgreen'
    response["highlighted_citations"] = highlighted_citations
    return response
...
def extract_citations(response_text):
    ...
    # Pattern for square bracket citations (e.g., [1], [2], etc.)
    citation_pattern = r'\[(\d+)\]'  # Matches numbers in square brackets
    
    # Pattern for URLs
    url_pattern = r'https?://[^\s]+'  # Matches URLs starting with http:// or https://
    
    # Pattern for text-based citations (e.g., (Author, Year), etc.)
    text_citation_pattern = r'\([A-Za-z\s]+, \d{4}\)'  # Matches (Author, Year)

    # Find all patterns in the response text
    citations.extend(re.findall(citation_pattern, response_text))
    citations.extend(re.findall(url_pattern, response_text))
    citations.extend(re.findall(text_citation_pattern, response_text))
    
    return citations
# def send_audit_entry(user_id, app_name, status, model_name, input_tokens, output_tokens, total_tokens, response_time, table_name, step_name, user_question, bot_answer):
#     # Preparing the audit data and ensuring data types and truncating long text fields for testing
#     audit_data = {
#         "user_id": str(user_id),
#         "app_name": app_name,
#         "status": status,
...
#         "response_time": round(float(response_time), 2),  # Rounded to 2 decimal places
#         "table_name": table_name,
#         "step_name": step_name,
#         "user_question": user_question[:100],  # Limit length to 100 characters
#         "bot_answer": bot_answer[:100]  # Limit length to 100 characters
#     }
#     logging.info("audit_data")
#     logging.info(audit_data)
 
#     try:
#         response = requests.post(AUDIT_ENDPOINT, headers={"Content-Type": "application/json"}, json=audit_data)
...
async def send_chat_request(request_body, request_headers):
    ...
    print("model_arguments", model_args)
...
async def stream_chat_request(request_body, request_headers):
    ...
    print(f"Received response (stream): {type(response)}")
...
async def complete_chat_request(request_body, request_headers):
    if app_settings.base_settings.use_promptflow:
        ...
        print("response in if complete chat request",response)
        history_metadata = request_body.get("history_metadata", {})
        ...
        response, apim_request_id = await send_chat_request(request_body, request_headers)
        print("response in else complete chat",response)
        history_metadata = request_body.get("history_metadata", {})
...
async def conversation_internal(request_body,request_headers):
    try:
        if app_settings.azure_openai.stream and not app_settings.base_settings.use_promptflow:
            ...
            print("response of conversontial",response)
            ...
            print("result in else chat complete",result)
...
async def generate_content(conversation_messages) -> str:
    ...
    try:
        ...
        print("respose for azure",response)
