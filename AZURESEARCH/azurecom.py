...
class SharePointItem(BaseModel):
    ...
    site_url:Optional[str] = Field(..., description="Site URL for SharePoint API")
    ...
    exclude_drive_id: Optional[str] = Field(None, description="ID of the SharePoint drive to exclude (optional)")
...
def get_sharepoint_pages(site_id, access_token):
    ...
    if response.status_code == 200:
        ...
        return []
def get_sharepoint_id(site_url, access_token):
...
def get_drive_id(site_url, access_token):
    site_id = get_sharepoint_id(site_url, access_token)
    if not site_id:
        return None
...
def index_document_to_azure_search(file_name, file_content, file_id, permissions, group_members, web_url):
    ...
    try:
        ...
        document = {"id": str(file_id), "fileName": file_name, "content": content_str, "permissions": permissions, "accessList": [str(member) for member in group_members], "webUrl": web_url, "contentEmbeddings": embeddings_list[0] if embeddings_list else []}
...
def get_sharepoint_details(item: SharePointItem):
    ...
    siteurls=item.site_url.split(",") if item.site_url else []
    sharepoint_site_id= get_sharepoint_id(item.site_url, item.ACCESS_TOKEN)
    if item.configuration == "drive":
        if  item.SHAREPOINT_DRIVE_ID  and item.site_url:
            ...
            drives = get_drive_id( item.site_url,item.ACCESS_TOKEN)
            fields = ["content", "weburl", "fileName","folderName"]
            create_or_update_index(search_service_endpoint, admin_key, index_name, fields) 
            all_drive_items = []  
            for drivess in drives:
                drive_id = drivess["id"]
                # print("drive_id",drive_id)
                 
                if item.folder_id:
                       drive_items = get_sharepoint_items(item.ACCESS_TOKEN, item.folder_id, drive_id)
                       print("drive_id in if part",drive_id)
                else:
                      drive_items = get_sharepoint_items(item.ACCESS_TOKEN, None, drive_id)
                      print("drive_id in else part",drive_id)
                
                
            # Extract fields dynamically from the files in the drive
                
                index_drive_folder(drive_id, item.ACCESS_TOKEN)  
            return {"drive_items": drive_items}      
        ...
        if item.list_id and item.site_url:
         ...
         # Index data for all lists within the SharePoint site
            ...
            if item.site_url:
                pages = get_sharepoint_pages(sharepoint_site_id, item.ACCESS_TOKEN)
