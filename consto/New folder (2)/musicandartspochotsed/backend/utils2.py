...
def qplugin(inputText, appName):
    try:
        payload = {"appname": appName, "user_query": inputText}
        url = "api/qplugin_trigger"
        r = requests.post(url, data=payload)
        if r.status_code != 200:
            logging.error(f"Error executing qplugin: {r.status_code} {r.text}")
            return {}
        r = r.json()
        logging.debug(f"QPLUGIN RESPONSE: {r}")
        return r
    except Exception as e:
        logging.error(f"Exception in qplugin: {e}")
        return {}


def filterEntitiesSources(inputText, appName):
    data = qplugin(inputText, appName)
    ...
    for key, value in data.items():
        ...
        if key.lower() == "sources":
            sourceExpr = []
            for source in value:
                sourceExpr.append("sources eq '{}'".format(source))
            filter = "(" + " or ".join(sourceExpr) + ")"
        else:
            filter = "{}/any(g: search.in(g, '{}', ','))".format(key, ",".join(value))
        filterString.append(filter)
    return " and ".join(filterString)
