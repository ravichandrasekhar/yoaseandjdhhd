...
import Chatlogoai from "../../assets/Chatlogoai.svg";
import Addicon from "../../assets/Addicon.svg";
import Closeicon from "../../assets/Closeicon.svg";
import Activeadd from "../../assets/Activeadd.svg";
import Activeclose from "../../assets/Activeclose.svg";
...
const Chat = () => {
    ...
    const onViewSource = (citation: Citation) => {
        ...
        if (citation.url && !citation.url.includes("blob.core") && !citation.url.includes("Riversand PIM")) {
    ...
    const imgDisabledButton = () => {
         if(isLoading || (messages && messages.length === 0) || clearingChat || appStateContext?.state.chatHistoryLoadingState === ChatHistoryLoadingState.Loading){
             return "white"
         }
         else{
            return "black"
                ...
                <Stack horizontal className={styles.chatRoot}>
                    <div className={styles.chatContainer}>
                        ...
                        <Stack horizontal className={styles.chatInput}>
                            ...
                            <div style={{display:"flex"}}>
                                <div style={{padding:'15px 6px 0px 0px'}}> 
                                ...
                                //         root: {
                                //             color: '#FFFFFF',
                                //             background:'rgb(39, 92, 226) !important'
                                //         },
                                //         rootHovered: {
                                //             color: '#FFFFFF',
                                //           },
                                ...
                                //         }
                                //     }}
                                //     className={styles.newChatIcon+ " customaddIcon"}
                                //     iconProps={{ iconName: 'Add' }}
                                //     onClick={newChat}
                                //     disabled={disabledButton()}
                                //     aria-label="start a new chat button"
                                // />
                                <div className={disabledButton()?styles.addicon:styles.activeadd}>
                                 <img title="start"
                                style={{height:'25px', width:'25px'}}
                                    src={disabledButton()?Addicon:Activeadd}
                                    aria-hidden="true"
                                    onClick={newChat}
                                />
                                ...
                                <div style={{padding:'15px 6px 0px 0px'}}>
                                <div className="tooltip-container">
                                <div className={disabledButton()?styles.addicon:styles.closeadd} >
                               
                                <img 
                                style={{height:'25px', width:'25px'}}
                                    src={disabledButton()?Closeicon:Activeclose}
                                    aria-hidden="true"
                                ...
                                {/* <CommandBarButton
                                    ...
                                    styles={{
                                        ...
                                        root: {
                                            color: '#FFFFFF',
                                            background: "radial-gradient(106.04% 106.06% at 100.1% 90.19%,#f0d91a 47.63%,#747253 119%)!important"
                                    ...
                                    className={appStateContext?.state.isCosmosDBAvailable?.status !== CosmosDBStatus.NotConfigured ? styles.clearChatBroom +" customClear" : styles.clearChatBroomNoCosmos +" customClearCosmos"}
                    ...
                    {messages && messages.length > 0 && isCitationPanelOpen && activeCitation && (
                        <Stack.Item className={styles.citationPanel} tabIndex={0} role="tabpanel" aria-label="Citations Panel">
                            <Stack aria-label="Citations Panel Header Container" horizontal className={styles.citationPanelHeaderContainer} horizontalAlign="space-between" verticalAlign="center" style={{borderBottom:'1px solid #ECECEC'}}>
                                <span aria-label="Citations" className={styles.citationPanelHeader}>Citations</span>
                                <IconButton iconProps={{ iconName: 'Cancel' }} aria-label="Close citations panel" className={styles.custombutton} onClick={() => setIsCitationPanelOpen(false)} />
                            </Stack>
                            <h5 className={
                                activeCitation.url && !activeCitation.url.includes("Riversand PIM")?
                                styles.citationPanelTitle:styles.citationPanelTitleReversand} tabIndex={0} 
                            title={activeCitation.url && !activeCitation.url.includes("blob.core") ? activeCitation.url : activeCitation.title ?? ""} 
                            onClick={() => onViewSource(activeCitation)}>{activeCitation.title}</h5>
                            <div tabIndex={0}>
                                <ReactMarkdown
                                    linkTarget="_blank"
